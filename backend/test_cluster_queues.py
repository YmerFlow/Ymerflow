#!/usr/bin/env python
"""Unit test for GET /utilities/cluster-queues (docs/plans/done/cluster-queue-widget.md).

Runs against an in-memory async SQLite DB with a faked K8sClient.list_workloads, exercising the
security-critical logic directly on the endpoint function: Kueue admission ordering, membership
(not creator) driven visibility, server-side redaction, and per-cluster graceful degradation.
...
class _FakeClient:
    def __init__(self, workloads=None, raise_exc=None):
...
async def _seed_and_run():
    ...
    async with Session() as db:
        ...
        # proc1: in P1, STARTED BY B, running/admitted — A must still see it in FULL (membership).
        proc1 = Process(id="proc1", name="invert_aem", type="aem_inversion",
                        environment_id="env1", project_id="p1")
        # proc3: in P1, pending — A member, full.
        proc3 = Process(id="proc3", name="grid_mag", type="mag_gridding",
                        environment_id="env1", project_id="p1")
        # proc2: in P2, pending — A is NOT a member, redacted.
        ...
        pv3 = ProcessVersion(process_id="proc3", version=2, parameters={},
                             state=ProcessState.QUEUED, k8s_job_name="process-proc3-v1",
                             k8s_cluster_id="c1", resource_requests={"cpu": "500m", "memory": "1Gi"},
                             deadline_seconds=1800)
        db.add_all([pv1, pv2, pv3])
        await db.flush()
        ...
        c1_workloads = [
            ...
            # foreign workload — no matching ProcessVersion, resources from podSets only
            _workload("process-ghost-v9", admitted=False, created="2026-01-01T00:02:00Z",
                      pod_requests={"cpu": "4000m", "memory": "16Gi"}),
        ]

        clients = {
            "c1": _FakeClient(workloads=c1_workloads),
...
def test_cluster_queues():
    ...
    # position 0 = admitted proc1; then pending proc2 (00:00) < proc3 (00:01) < ghost (00:02)
    assert queue[0]["process_id"] == "proc1"
    assert queue[1]["state"] == "waiting"

    # (b) state reflects admission.
    assert queue[0]["state"] == "running"
    assert all(q["state"] == "waiting" for q in queue[1:])
    ...
    # proc3 (position 2) is also in p1 → full.
