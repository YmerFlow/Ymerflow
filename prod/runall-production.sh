...
REGISTRY_URL=$(env/bin/python "${PROJECT_ROOT}/backend/bin/yf-registry-image-prefix" \
    "${REGISTRY_PROTOCOL}" "${REGISTRY_CONFIG_JSON}")

echo "  Pushed ${BACKEND_IMAGE}"
echo "  Pushed ${FRONTEND_IMAGE}"

# ── Step 6: ymerflow-backend-config / nagelfluh-backend-secret ────────────────
# These carry the app's workload-level config/secrets. They are created here as the INPUT the
# in-cluster yf-deploy-app Job (Step 12) reads via envFrom; yf-deploy-app hands their
...
# provider's expose_app() (yf-deploy-app threads them through app_config).
#
...
EOF
echo "  ymerflow-backend-config applied"
...
# the ymerflow-backend-config/secret created above via envFrom, resolves the default Cluster's
